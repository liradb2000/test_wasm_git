...
// console.log(readCookie('partial_token'))
let accessToken = "78cd1976";
XMLHttpRequest.prototype._open = XMLHttpRequest.prototype.open;
XMLHttpRequest.prototype.open = function (method, url, async, user, password) {
  this.withCredentials = true;
  this._open(method, url, async, user, password);
  this.setRequestHeader("Authorization", `Token ${accessToken}`);
};
...
onmessage = async (msg) => {
  ...
  if (msg.data.accessToken) {
    ...
    currentRepoRootDir = msg.data.url.substring(
      msg.data.url.lastIndexOf("/") + 1
    );
    ...
    FS.syncfs(true, () => {
      if (
        FS.readdir(`/${currentRepoRootDir}`).find((file) => file === ".git")
      ) {
